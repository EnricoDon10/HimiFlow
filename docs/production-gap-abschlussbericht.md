...
| 3 Pagination/Filter | `SavingsListDtos`, Savings-API, All-/My-Savings-Komponenten | Serverseitige Filter und standardmäßig begrenzte Seiten schützen API und UI; die bewusst auswählbare Option `pageSize=0` zeigt alle gefilterten Datensätze in einer Seite. | `SavingsPaginationTests`, Browser-Abnahme |
