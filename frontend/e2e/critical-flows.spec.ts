...
const adminTemporaryPassword = 'E2e!Start9Password#2026';
const adminPassword = 'E2e!Admin9Password#2026';
const fachPassword = 'E2e!Fach9Password#2026';
...
test('SystemAdmin Erstlogin erzwingt Passwortwechsel und legt FachAdmin an', async ({ page }) => {
  ...
  await page.getByLabel('Passwort').fill(adminTemporaryPassword);
  ...
  await page.getByLabel('Rolle').selectOption('FachAdmin');
...
test('FachAdmin pflegt Stammdaten und erfasst, bearbeitet und löscht eine Einsparung', async ({ page }) => {
  await page.goto('/login');
  await page.getByLabel('Benutzername').fill('e2e.fachadmin');
  await page.getByLabel('Passwort').fill(fachAdminTemporaryPassword);
  await page.getByRole('button', { name: 'Anmelden' }).click();
  await expect(page).toHaveURL(/change-password/);
...
test('Lizenzseite und Logout sind erreichbar', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: /Lizenzverwaltung/ })).toBeVisible();
  await page.getByRole('button', { name: 'Abmelden' }).click();
